...
function getWeightNumber(weight: string) {
  if (weight === 'normal') return NORMAL_WEIGHT
  if (weight === 'bold') return BOLD_WEIGHT
  return Number(weight)
}

/**
 * Get the distance from normal (400) weight for the provided weight.
 * If it's not a variable next-font we can just return the distance.
 * If it's a variable next-font we need to compare its weight range to 400.
 */
function getDistanceFromNormalWeight(weight?: string) {
  if (!weight) return 0

  // If it's a variable next-font the weight is defined with two numbers "100 900", rather than just one "400"
  ...
  // If the weight doesn't have have a second value, it's not a variable next-font
  // If that's the case, just return the distance from normal weight
  if (!secondWeight) {
    return firstWeight - NORMAL_WEIGHT
  }

  // Normal weight is within variable next-font range
  if (firstWeight <= NORMAL_WEIGHT && secondWeight >= NORMAL_WEIGHT) {
    return 0
  }

  // Normal weight is outside variable next-font range
  // Return the distance of normal weight to the variable next-font range
...
/**
 * If multiple next-font files are provided for a next-font family, we need to pick one to use for the automatic fallback generation.
 * This function returns the next-font file that is most likely to be used for the bulk of the text on a page.
 *
 * There are some assumptions here about the text on a page when picking the next-font file:
 ...
 * - If two next-font files have the same distance from normal weight, the thinner one will most likely be the bulk of the text
 */
export function pickFontFileForFallbackGeneration<T extends { style?: string; weight?: string }>(
  ...
  return fontFiles.reduce((usedFontFile, currentFontFile) => {
    ...
    // Prefer the thinner next-font if both have the same absolute distance from normal weight
