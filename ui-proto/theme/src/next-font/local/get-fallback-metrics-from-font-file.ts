...
// The next-font metadata of the fallback fonts, retrieved with fontkit on system next-font files
...
/**
 * Calculate the average character width of a next-font file.
 * Used to calculate the size-adjust property by comparing the fallback average with the loaded next-font average.
 */
function calcAverageWidth(font: Font): number | undefined {
  try {
     ...
     * We also have to take word length into account, because the next-font's space width usually differ a lot from other characters.
    ...
    // Check if the next-font file has all the characters we need to calculate the average width
    ...
    // Could not calculate average width from the next-font file, skip size-adjust
    return undefined
  }
}
...
/**
 * Given a next-font file and category, calculate the fallback next-font override values.
 * The returned values can be used to generate a CSS @next-font-face declaration.
 *
 * For example:
 * @next-font-face {
 *   next-font-family: local-next-font;
 *   src: local(Arial);
 *   size-adjust: 90%;
 * }
 *
